...
export async function smartGenerateSchedule(requestData) {
  ...
  export async function fetchRoles(search = "", page = 0, size = 100) {
    ...
    // Бэкенд возвращает Page -> { content: [...], totalElements, ... }
    if (response.data && Array.isArray(response.data.content)) {
      return response.data.content;
    }
    // Если вдруг вернётся просто массив
...
export async function fetchProgramsByDepartment(departmentId) {
  ...
  export const deleteProgramFromDepartment = async (programId, departmentId) => {
    const response = await API.delete(
      "/api/program-department/unlink",
      { params: { programId, departmentId } }
    );
    return response.status === 204;
  };

  export async function addProgramToDepartment(programId, departmentId, isPrimary = false, notes = "") {
    const response = await API.post("/api/program-department/link", {
      programId,
      departmentId,
      isPrimary,
      notes
  ...
  export async function updateProgramSubject(id, academicYear, hoursPerWeekForProgram) {
    const response = await API.put(`/api/program-subjects/${id}`, {
      academicYear,
      hoursPerWeekForProgram
    });
    return response.data;
  }

  export async function unlinkSubjectFromProgram(programId, subjectId, academicYear) {
    const response = await API.delete("/api/program-subjects/unlink", {
      params: { programId, subjectId, academicYear }
    });
    return response.status === 204;
  }
  ...
  /**
   * Загружает новые файлы для сущности (добавляет к существующим, не удаляя старые)
   */
  ...
  /**
   * Полностью заменяет все файлы сущности новыми (удаляет старые, загружает новые)
   */
  ...
  // ================= ROOMS API (специфичные методы, используют универсальные файловые) =================
  ...
  // Получить первое изображение комнаты (удобная обёртка)
...
export async function fetchStudentGradeDetails(studentId, subjectId, periodId = null) {
  ...
  const response = await API.get(
    `/api/journal/student/${studentId}/grades/${subjectId}/details?${params.toString()}`
  );
  return response.data;
}
...
export async function getSystemSettings() {
  ...
  return response.data;   // сервер возвращает объект настроек
}
...
export async function getPublicSettings() {
  ...
  return response.data;   // вернёт объект { orgName, email, phone, ... }
}
